...
3. If no error return ONLY the SQL generated
...
if llm_type == "OPENAI":
    ...
    llm = OCIGenAIModel(
        ...
        compartment_id="ocid1.compartment.oc1..aaaaaaaa") #CHANGE ACCORDING YOUR CREDENTIALS
...
input_file = 'cache/dataflow_cache_step_step3_updated.jsonl'
output_file = 'cache/dataflow_cache_step_step3_q_a.jsonl'  # Or overwrite by setting the actual sql translated
...
with open(input_file, 'r') as infile, open(output_file, 'w') as outfile:
    for line in infile:
        ...
        if line:  
            ...
            conversation.append_user_message(data['question'])
            logger.info(f"\nUser   >>> {data['question']}")
            ...
            while True:
                ...
                if isinstance(status, FinishedStatus):
                    data['NL2SQL']= reply.content
                    outfile.write(json.dumps(data) + '\n')
                    logger.info("FinishedStatus")
                    break
                if isinstance(status, UserMessageRequestStatus):
                    logger.info("UserMessageRequestStatus")
